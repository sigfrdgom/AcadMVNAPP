...
echo copio el ear
cd /var/lib/jenkins/workspace/sigfridPipeline/compose/ && docker-compose stop
echo error si no esta activo
docker rm compose_glassfish_1
echo eliminando viejo contenedor
cd /var/lib/jenkins/workspace/sigfridPipeline/compose/glassfish/ && ./glass.sh
echo contruyendo la nueva imagen
cd /var/lib/jenkins/workspace/sigfridPipeline/compose/ && docker-compose up -d
